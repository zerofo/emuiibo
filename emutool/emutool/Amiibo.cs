...
namespace emutool
{
    public static class AmiiboUtils
    {
        ...
        public class Amiibo
        {
            ...
            public void Save(string dir, bool random_uuid, bool save_image)
            {
                try
                {
                    ...
                    Utils.SaveJSON(Path.Combine(dir, "amiibo.json"), json);
                    Utils.CreateEmptyFile(Path.Combine(dir, "amiibo.flag"));
                    if(save_image)
                    {
                        Utils.SaveFromURL(OriginalAmiibo.ImageURL, Path.Combine(dir, "amiibo.png"));
                    }
                }
                catch(Exception ex)
                {
        ...
        {
            Utils.Unless(api_amiibo != null, "Invalid input amiibo");
            ...
            try
            {
                ...
                Utils.Unless(id.Length == 16, "Invalid amiibo ID");
                ...
                amiibo.Id.CharacterId.GameCharacterId = Utils.ReverseUInt16(character_game_id_be);
                ...
                Utils.Rng.NextBytes(amiibo.Uuid);
                amiibo.Uuid[7] = 0;
                amiibo.Uuid[8] = 0;
                amiibo.Uuid[9] = 0;
            }
